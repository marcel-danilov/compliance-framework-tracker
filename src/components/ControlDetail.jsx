...
function DocumentPicker({ controlId, linkedIds, onClose }) {
  ...
  return (
    ...
    >
      <div className="glass-modal rounded-2xl w-full max-w-md p-5 shadow-modal">
        <h3 className="font-semibold text-brand-800 dark:text-brand-100 mb-1 tracking-tight">{t.documentPicker.title}</h3>
        <p className="text-sm text-brand-400 dark:text-brand-400 mb-4">{t.documentPicker.subtitle}</p>

        {!allDocs || allDocs.length === 0 ? (
          <p className="text-sm text-brand-400 dark:text-brand-400 py-6 text-center">
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto mb-4 -mx-1 px-1">
            {allDocs.map((doc) => (
              <label
                key={doc.id}
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-brand-100/50 dark:hover:bg-brand-700/50 cursor-pointer transition-colors"
              >
                <input
                  type="checkbox"
                ...
                />
                <div className="w-7 h-7 rounded-lg bg-brand-100/60 dark:bg-brand-700/60 flex items-center justify-center flex-shrink-0">
                ...
                <div className="min-w-0">
                  ...
                  {doc.description && (
                    <p className="text-xs text-brand-400 dark:text-brand-400 truncate">{doc.description}</p>
                  )}
                </div>
              </label>
            ))}
...
export default function ControlDetail({ control, onClose }) {
  ...
  return (
    <div className="w-96 border-l border-brand-200/40 dark:border-brand-700/40 glass-card flex flex-col h-full flex-shrink-0 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-brand-200/40 dark:border-brand-700/40 flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <code className="text-xs text-brand-500 dark:text-brand-300 bg-brand-100/60 dark:bg-brand-700/60 px-1.5 py-0.5 rounded-md font-mono border border-brand-200/50 dark:border-brand-600/50">
              {control.controlId}
            </code>
            {control.category && (
              <span className="text-xs px-1.5 py-0.5 bg-brand-100/60 dark:bg-brand-700/60 text-brand-500 dark:text-brand-300 rounded-md border border-brand-200/50 dark:border-brand-600/50 font-medium">
                {control.category}
              </span>
            )}
          </div>
          <h3 className="font-semibold text-brand-800 dark:text-brand-100 text-sm leading-snug tracking-tight">
            {control.name}
          </h3>
        </div>
        <button
          onClick={onClose}
          className="text-brand-400 hover:text-brand-700 dark:hover:text-brand-100 flex-shrink-0 mt-0.5 transition-colors p-1 rounded-lg hover:bg-brand-100/50 dark:hover:bg-brand-700/50"
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-brand-600 dark:text-brand-300 leading-relaxed">{control.description}</p>
        </div>
        ...
        <div>
          ...
          {!linkedDocs || linkedDocs.length === 0 ? (
            <button
              onClick={() => setShowPicker(true)}
              className="w-full border-2 border-dashed border-brand-200/50 dark:border-brand-600/50 rounded-xl py-4 text-center text-xs text-brand-400 dark:text-brand-400 hover:border-azure-300 hover:text-azure-500 hover:bg-brand-50/50 dark:hover:bg-brand-700/40 transition-colors"
            >
            ...
            <div className="space-y-1.5">
              {linkedDocs.map((doc) => (
                <div
                  key={doc.id}
                  className="flex items-center gap-2.5 px-3 py-2 bg-brand-50/60 dark:bg-brand-700/50 rounded-xl group"
                >
                  <div className="w-6 h-6 rounded-md bg-white/70 dark:bg-brand-800/60 flex items-center justify-center flex-shrink-0 border border-brand-200/50 dark:border-brand-600/50">
                    <FileTypeIcon fileName={doc.fileName} className="h-3 w-3 text-brand-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-brand-700 dark:text-brand-100 truncate">{doc.name}</p>
                    {doc.description && (
        ...
        <div>
          <label
            ...
            className="block text-xs font-semibold text-brand-400 uppercase tracking-widest mb-2"
          >
          ...
          <textarea
            ...
            className="w-full border border-brand-200/50 dark:border-brand-600/50 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-azure-400/50 focus:border-azure-400 resize-none text-brand-700 dark:text-brand-100 bg-white/60 dark:bg-brand-700/50 backdrop-blur-sm placeholder-brand-300 dark:placeholder-brand-500 transition-all leading-relaxed"
          />
        </div>
      </div>

      {/* Footer */}
      <div className="px-5 py-3.5 border-t border-brand-200/40 dark:border-brand-700/40 flex items-center justify-between">
